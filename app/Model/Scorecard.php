...
class Scorecard extends AppModel {
	...
	public function generateMVP($offset, $limit) {
		$counter = 0;
		$games = $this->Game->find('all', array('contain' => 'Scorecard', 'order' => array('id' => 'asc'),'limit' => $limit, 'offset' => $offset));
		foreach($games as $game) {
			$red_medic_alive = false;
			$green_medic_alive = false;
			$winner = $game['Game']['winner'];

			foreach($game['Scorecard'] as $score){
				if($score['position'] == 'Medic' && $score['lives_left'] > 0 && $score['team'] == 'Red')
					$red_medic_alive = true;

				if($score['position'] == 'Medic' && $score['lives_left'] > 0 && $score['team'] == 'Green')
					$green_medic_alive = true;
			}
			foreach($game['Scorecard'] as $score){
				$mvp = 0;
				...
				//did you win?
				if($winner == $score['team'])
					$mvp += 2;
				
				//bases for 2 points each
				$mvp += $score['bases_destroyed'] * 2;
				
				//get dat 5-chain
				$mvp += $score['nukes_detonated'];
				
				//push the little button
				$mvp += $score['scout_rapid'] * 1;
				$mvp += $score['life_boost'] * 3;
				$mvp += $score['ammo_boost'] * 3;
				
				//accuracy bonus
				$mvp += round($score['accuracy'] * 10,1);			
				
				//hit diff > 1 = bonus
				if($score['shot_opponent']/(max($score['times_zapped'],1)) >= 1)
					$mvp += round($score['shot_opponent']/(max($score['times_zapped'],1)),2)*3;
					
				//survival bonus
				if($score['lives_left'] > 0 && $score['position'] != 'Medic')
					$mvp += 1;

				if($score['lives_left'] > 0 && $score['position'] == 'Medic')
					$mvp += $score['lives_left'];

				if($score['team'] == 'Red' && $red_medic_alive)
					$mvp += 2;
					
				if($score['team'] == 'Green' && $green_medic_alive)
					$mvp += 2;
				
				//get missiles
				$mvp += $score['missiled_opponent'];

				//don't get missiled dummy
				$mvp += $score['times_missiled'] * -1;					

				//medic tears are scrumptious
				$mvp += $score['medic_hits'];

				//dont be a venom
				$mvp += $score['own_medic_hits'] * -1;
				
				//make commanders cry
				$mvp += $score['nukes_canceled'] * 3;				

				//No.  Stahp.
				$mvp += $score['own_nuke_cancels'] * -3;
				
				//more venom points
				$mvp += $score['missiled_team'] * -3;
				
				//lose 5 points for every penalty in competitive games only
				if($score['type'] == 'league' || $score['type'] == 'tournament') {
	...
	public function generateGames() {
		...
		$scores = $this->query("SELECT green.game_datetime, green.type, green.score, red.score, green.team_elim, red.team_elim, green.pdf_id, green.league_id, green.center_id
			FROM (
		...
		if(isset($state['gametype']) && $state['gametype'] != 'all') {
			...
			if($state['gametype'] == 'league') {
				...
				if(isset($state['show_finals']) && $state['show_finals'] == 'true' && isset($state['show_rounds']) && $state['show_rounds'] == 'true') {
        			...
        			$subQuery->value = "Scorecard.game_id IN (SELECT game_id FROM lfstats.league_games WHERE league_id='{$state['leagueID']}')";
       		 		$conditions[] = $subQuery;					
				} elseif(isset($state['show_finals']) && $state['show_finals'] == 'true') {
					$subQuery = new stdClass();
					$subQuery->type = "expression";
        			$subQuery->value = "Scorecard.game_id IN (SELECT game_id FROM lfstats.league_games WHERE is_finals = 1 AND league_id='{$state['leagueID']}')";
       		 		$conditions[] = $subQuery;
		...
		if(isset($state['gametype']) && $state['gametype'] != 'all') {
			...
			if($state['gametype'] == 'league') {
				...
				if(isset($state['show_finals']) && $state['show_finals'] == 'true' && isset($state['show_rounds']) && $state['show_rounds'] == 'true') {
					$subQuery = new stdClass();
					$subQuery->type = "expression";
        			$subQuery->value = "game_id IN (SELECT game_id FROM lfstats.league_games WHERE league_id='{$state['leagueID']}')";
       		 		$conditions[] = $subQuery;					
				} elseif(isset($state['show_finals']) && $state['show_finals'] == 'true') {
					$subQuery = new stdClass();
					$subQuery->type = "expression";
        			$subQuery->value = "game_id IN (SELECT game_id FROM lfstats.league_games WHERE is_finals = 1 AND league_id='{$state['leagueID']}')";
       		 		$conditions[] = $subQuery;
				} elseif(isset($state['show_rounds']) && $state['show_rounds'] == 'true') {
					$subQuery = new stdClass();
					$subQuery->type = "expression";
